...
def get_content(html,url): #полусение конкретной информации в зависимости от того, какие параметры мы ввели в предыдущую функцию и что нам вернули
    ...
    items = soup.find_all('div', class_=re.compile(r'^row bg'))
    ...
    for item in items:
        ...
        for message in count_messages:
            count = message.get_text(strip=True)  # получаем весь текст внутри <dd>, очищенный от лишних пробелов
            if 'Сообщений' in count:
                # ищем число в тексте с помощью регулярного выражения
                count_messages = re.search(r'\d+', text)
            else:
                # если число не найдено, можно добавить None или пропустить
                count_messages = 0
...
def read_from_document():
    with open('D:\Documents\GitHub\Psychic-Parser\datasets\links_to_separation_anxiety_info.csv', 'r', newline='') as file:
